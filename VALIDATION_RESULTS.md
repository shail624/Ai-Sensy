...
PASS: reproduced against live MySQL through the real ORM path before any fix existed —
`(3819, "Check constraint 'ck_phone_numbers_ck_phone_quality' is violated.")` — with the same-batch
GREEN number's update lost alongside the rejected one, confirmed by re-querying after rollback.

PASS: migration verified up → down → up against live MySQL. Downgrade preserves the phone-number
row and nulls only `quality_rating`; confirmed by direct row read (`id`, `phone_number_id` both
intact, `quality_rating` NULL) rather than assumed from the migration's own code.

PASS: a new backend test reproduces the owner's exact two-number batch (GREEN + UNKNOWN in one
sync) and fails against the pre-fix model with the identical constraint-violation class SQLite
raises for the same constraint — proven by stashing the model change and re-running the test before
restoring it, not merely written and trusted.

PASS: backend lint and strict mypy (332 files) clean. Frontend **1,020 passed** (1,019 → 1,020 for
this milestone's own new test), TypeScript and ESLint clean.

PENDING – Host Machine Validation: the fix has not yet run against the owner's real
`1061162763007187` WABA. The owner's own deploy needs `docker compose ... up migrate` (or the
Windows PowerShell runbook's equivalent) re-run to reach `0070` before a sync against the real
...
PASS: backend **1,773 passed, 0 failed, 0 skipped** — the count already reflects everything GSHEET-02
through UI-REF-10 added; this milestone's own change is comment-only on the backend side. Ruff and
strict mypy (332 files) both clean.

PASS: frontend **1,019 passed, 0 failed** (1,018 → 1,019; one new regression test). TypeScript and
ESLint clean. Verified live: signed in against the rebuilt production bundle, filtered Live Chat to
`status=open`, and read the rendered button's `aria-label` directly — `"Filters, 1 active"` — with
the visible badge alongside it in a real screenshot, not only in jsdom.

PASS: the new test fails against the pre-fix file. Stashed `ConversationFilters.tsx` alone (kept the
test), re-ran, confirmed `getByRole("button", { name: "Filters, 1 active" })` cannot find the
element against the old code, then restored the fix and confirmed it passes.

PASS: `git merge-base --is-ancestor claude/sweet-darwin-5i66uq origin/main` before the fast-forward,
so nothing was rebased, merged with conflict-resolution guesses, or otherwise put at risk of losing
...
PASS: frontend **1,011 tests across 61 files**, zero failures, 27.5s. TypeScript and ESLint clean.
The 42 inbox tests — `inbox-counts.test.tsx` among them — pass **unmodified**, which is the point:
the accessible name did not change, so the assertions that matter did not need to.

PASS: verified in a real signed-in browser against the production build, not only in jsdom. The tab
strip reports `["Active(0)","Requesting(0)","Intervened(0)"]`, rendering as
`ACTIVE (0)  REQUESTING (0)  INTERVENED (0)`.

PASS: the archive is inert in Git. `git status` is clean after extracting 414 files into
`.reference/aisensy/`, and `.gitignore:82` covers the directory.

PENDING – Owner acceptance: five differences remain on this screen (filter control, collapse
control, search button, empty-list illustration, conversation-pane ground). Two of those are
reference assets and will be matched by position and treatment with original artwork, never copied.

PENDING – Host Machine Validation: unchanged. No completion percentage increase — one screen's tab
strip is not a module.
...
PASS: §3 through §7 executed on the owner's Windows host — build, migrate to `0068`, owner
creation through the `bootstrap` profile, ten services started with `api` and `frontend` healthy,
and `/ready` 200 on the first poll.

PASS: DEPLOY-02 disproven in the field. The environment file carried no `WAHA_*` variable of any
kind, which is exactly the configuration that aborted `build` before the guards moved into the
profile.

PASS: DEPLOY-01 disproven in the field. `Owner created` on the first attempt, via
`--profile bootstrap run --rm bootstrap`.

PENDING – Host Machine Validation: SPA sign-in, the §8 smoke sequence, TLS, the browser matrix, and
OPS-02's fleet view on this host remain unexecuted. WhatsApp sending is inert on this deployment by
configuration, not by defect: `META_APP_SECRET` is a generated placeholder.
...
PASS: backend **1,770 passed, 0 failed, 0 skipped** in 632s at INFO with live MySQL 8 and Redis
(1,764 → 1,770; six new worker-registry tests). Lint and strict `mypy` clean across **332** source
files.

PASS: §4–§8 of the deployment guide executed natively against real MySQL and Redis — empty schema →
**93 tables at `0068_attribute_required_and_active`** → `Owner created` → re-run reports `Owner
already exists` → uvicorn in `ENVIRONMENT=production` → `/health` ok → `/ready` **200** with
database, redis and storage all `up` → owner signs in through `POST /api/v1/auth/login` and receives
a token. This is the chain DEPLOY-01 repaired, now executed end to end rather than reasoned about.

PASS: read sweep across every parameterless `GET` on a brand-new deployment — **79 requests, zero
5xx and zero transport errors**. Distribution 200×61, 400×12, 422×3, 404×2, 403×1. All three
non-2xx singles were verified correct rather than assumed: the QR session route reports the provider
unconfigured, identity-conflicts reports the feature disabled, and the Meta webhook route fails
verification closed for a request carrying no `hub.*` parameters. The one slow response
(`/api/v1/activation-records`, 399 ms) was cold start: five repeats returned 5.6–8.0 ms.

PASS: the worker-registry defect was proven against the running system before any code changed —
`workers: []` from the endpoint, zero `worker:heartbeat:*` keys in Redis, `beat()` with no callers
in `app/`, and no Celery signal handler of any kind in the codebase.

PASS: the fix was verified against a real fleet, not a fixture: registration, refresh at a third of
the TTL, immediate deregistration on `SIGTERM`, and TTL reaping after `SIGKILL` — with the 17 queues
that have no consumer correctly reporting zero.

PASS: the wiring test fails against the defect it describes. With the import removed from
`celery_app`, its subprocess probe exits non-zero with `celery app does not import it`.

PENDING – Host Machine Validation: unchanged and unaffected — containers, TLS, the edge proxy and
the browser matrix still need a host with registry access. What this milestone establishes is that
the application-level first-deploy chain is sound; it does not establish that the Compose stack
starts, because no image can be built in this environment.
...
Canonical average unchanged at **87.5%**: hardening, not scope — and deliberately *not* the
"published documentation" item, which remains pending.

PASS: backend **1,754 passed, 0 skipped** at INFO against live MySQL 8 (1,752 → 1,754; two new
tests — the explorer still served outside production, and the setting winning in both directions).

PASS: the exposure was measured, not assumed — `/docs`, `/redoc` and the schema each returned
**200** to an unauthenticated request against the running API before the change.

PASS: `scripts/export_openapi.py --check` clean, so the contract the frontend is generated from is
unaffected by the mount.

PASS: backend lint and strict `mypy` across **331** source files. `.env.production.example`
documents the setting and the reasoning beside it.
...
Canonical average 87.3% → **87.4%**; median **90%**.

PASS: backend **1,752 passed, 0 skipped** at INFO against live MySQL 8 (1,747 → 1,752; five new
attribute tests, including the one that fixes the meaning of "required" by asserting a partial
update which *omits* it still succeeds).

PASS: migration `0068_attribute_required_and_active` applied **up / down / up** against the running
MySQL 8; both columns present after each upgrade and absent after the downgrade.

PASS: frontend **1,009 passed across 61 files** (1,007 → 1,009), ESLint, TypeScript and the
production build. OpenAPI regenerated at **247 paths** — two fields added to three schemas, no path
added — with the TypeScript client regenerated from it.
...
Canonical average unchanged at **87.2%**: this makes existing gates runnable rather than adding
scope.

PASS: stack stopped completely, then `./scripts/local_stack.sh` cold-started the API, worker and
production build in **23.8s** and returned; a second run with everything up returned in **0.06s**.

PASS: **5 of 5** browser tests against that stack — WCAG 2.1 AA over 24 routes light, 24 dark, 24 at
375px, the sign-in screen, and the owner journey end to end, in 1.9 minutes.

PASS: the three failure modes above were each observed before being fixed — a ten-minute hang with
every service healthy, a five-minute import timeout with no worker, and three `429`-driven failures
that presented as broken screens.

PENDING – Host Machine Validation: unchanged. This does not replace the containerised `deployed`
profile; it makes the same specs runnable where Docker is not.
...
Canonical average unchanged at **87.2%**.

PASS: **5 of 5** browser tests against the production build with the API on live MySQL 8.0.46,
Redis and a Celery worker — light theme, dark theme, 375px, the sign-in screen, and the owner
journey. The two that failed before this change (dark theme, phone width) now pass.

PASS: `--color-text-disabled` verified against **all nine** backgrounds it lands on in each theme —
`surface`, `canvas`, `surface-2`, `hover` and the five `-soft` tints — worst ratio **4.56:1** dark
and **4.60:1** light.

PASS: frontend **1,006 passed across 61 files**, ESLint, TypeScript and the production build.
...
Canonical average unchanged at **87.2%**: this is delivery quality, not scope.

PASS: entry chunk **276 kB → 185 kB raw, 70 kB → 50 kB gzip**; total JS 1,808 kB → 1,813 kB. The
`INEFFECTIVE_DYNAMIC_IMPORT` warning that every build printed is gone — build warnings now **0**.

PASS: `npm audit --omit=dev` **0 vulnerabilities** before and after; full audit **4 → 0**
(3 high, 1 moderate, all dev-only and all semver-safe). `package.json` unchanged; lockfile only.
`pip-audit`: no known vulnerabilities. Browser-test package: 0.

PASS: `schema.d.ts` regenerated after the toolchain bump is **byte-identical**, so the generated
client is provably unchanged.
...
Canonical average unchanged at **87.2%**: this restores a capability the rows already claimed
rather than adding one.

PASS: backend **1,744 passed, 0 skipped** at `log_level = "INFO"` against live MySQL 8.0.46
(1,742 → 1,744; the AST sweep and the campaign log line at INFO).

PASS: the bug is proven, not asserted. Against the pre-fix code the suite is green without
`--log-level` and reports **13 failures** with it, including
`test_a_campaign_can_bind_a_value_into_a_button_link` and
`test_manual_retry_resets_failed_recipients_and_redispatches`. The AST sweep fails against the
pre-fix code naming `app/services/campaign_batch_service.py:76`, and passes after.

PASS: `tests/contacts-import.spec.ts` against the production build with live MySQL, Redis and a
...
Canonical average 87.1% → **87.2%**; median **90%**.

PASS: `e2e/tests/accessibility.spec.ts` — **4 of 4** against the **production build** (`vite build`
served by `vite preview`) with the API on live MySQL 8.0.46 and a seeded organization of 31
contacts, 30 conversations and 191 messages. Signed-in WCAG 2.1 A/AA over **24 routes** in the light
theme, the same 24 in the dark theme, the same 24 at **375px**, and the sign-in screen: **zero
violations**. The same four also pass against the dev server.

PASS: the first run of that spec reported **151 failing nodes** (150 `color-contrast`, 1
`scrollable-region-focusable`) in the light theme and **61** in the dark. Both are now zero, and the
numbers are recorded here because a gate that has never failed proves nothing about what it checks.

PASS: frontend **1,006 passed across 61 files** (1,004 → 1,006; two tests for the new
`ScrollRegion` primitive), ESLint, TypeScript, browser-test TypeScript and the production build.

PASS: backend untouched by this milestone and unchanged at **1,742 passed, 0 skipped**.

PENDING – Host Machine Validation: a human visual and taste review, the browser matrix beyond
Chromium, screen-reader narrative quality, and target-host/production-scale commissioning.
...
Canonical average 87.0% → **87.1%**; median 88% → **90%**.

PASS: backend **1,742 passed, 0 skipped** against live MySQL 8.0.46 (1,735 → 1,742; seven new audit
tests), including the twelve live-MySQL migration tests — among them a full `base` → `head` upgrade
on a freshly created MySQL 8 database, which exercises `0066` from nothing.

PASS: migration `0066_audit_request_origin` applied **up / down / up** against the running MySQL 8
carrying real data. `user_agent` present after each upgrade and absent after the downgrade; **33**
audit rows and all **3** range partitions intact through every step.

PASS: integrity verdicts confirmed against that live database rather than only in tests — all **33**
pre-existing rows classify `verified_legacy` (none was ever recomputable from its own stored
content), and the first row written through the fixed path classifies `verified`. The probe row was
removed afterwards, leaving the trail at 33.

PASS: frontend **1,004 passed across 61 files** (1,001 → 1,004) with **zero unhandled errors**; the
run previously reported two escaping promise rejections and now reports none.

PASS: backend lint (`ruff check app tests scripts ../scripts`), strict `mypy` across **331** source
files, OpenAPI drift check clean at **247 paths**, ESLint, frontend TypeScript, browser-test
TypeScript and the production build.

PENDING – Host Machine Validation: authenticated representative-data visual/WCAG/browser review of
the audit entry dialog, as for every other screen.

Corrected while here: `MODULE_STATUS` recorded the contract as **246 paths**. It has carried **247**
since before this change — the export at the previous HEAD already reported 247 — so the row was
stale by one, not changed by this work. This milestone adds two fields to `AuditLogResponse` and no
path.
