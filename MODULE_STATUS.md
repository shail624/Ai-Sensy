...
- **Completion accounting:** this validation does not inflate feature percentages. Across the 31
  canonical module rows below, the simple unweighted average is **80.7%** and the recalculated
  median is **88%**.
...
|---|---:|---|---|---|
| Dashboard | 85% | UI-TASTE-03A replaces the messaging-led home with a permission-aware operational desk over real Reactivation, KYC, Task, Campaign, Inbox, Template and Analytics sources; DASH-01 opens it with whether WhatsApp will accept sends — each number's standing, what its tier permits, and when those stored figures were last checked | Authenticated representative-data visual/WCAG/browser review, a real rolling-window remaining-quota count, server-owned exact cross-domain aggregate totals beyond bounded source reads, and final route-performance commissioning. | Reactivation, KYC, Tasks, Campaigns, Inbox, Templates, Analytics |
...
| API | 99% | 244-path OpenAPI 3.1 contract with declared pagination and search across every collection that has them, in-place key rotation that preserves a key's identity, scopes and audit trail, with synchronized generated TypeScript; governed Reports, KYC, Campaign, Contacts and Reactivation personal/team views join campaign exports, Chat History, transcripts, Team workload, Analytics and Download Center resources | Remaining final-domain routes, usage logs/IP restrictions completeness, key regeneration/revocation UX, published documentation. | Each domain milestone, Download Center |
| Webhooks | 85% | Provider webhooks and operations surface complete; CORE-22 exposes the inbound delivery record and the dead-letter queue to operators, tenant-scoped through the route the delivery arrived on and without reproducing message payloads | Subscription governance, event replay (blocked on a public id the partitioned table cannot cheaply carry), outbound final-domain events, security/usage documentation. | Domain event taxonomy, API permissions |
| Google Sheets | 55% | GSHEET-01 reads a tab with a service account and stages it as an upload, so mapping, dedup, the per-row error report, job progress and audit are the existing contact import's rather than a second pipeline; the wizard gains the source beside the drop zone | The owner's service account and sheet share (the integration is otherwise complete), scheduled re-sync, export back to a sheet, and multi-tab batches. | Jobs, API keys/secrets, Contacts |
| WhatsApp Scan | 55% | SCAN-01 answers reachability from campaign delivery receipts — a delivery proves the number is reachable and Meta's 131026 says it is not, so ten of the eleven section 13 items are covered with nothing sent and no provider called | A direct scan of numbers never messaged still needs an approved provider (Meta has no lookup and unofficial Web enumeration stays excluded); business-account detection, batch upload/queue/retry and segment/export hand-off remain. | Owner-approved compliant method, Jobs, Segments, Download Center |
...
| Audit Timeline | 90% | CORE-07 exposes distinct Customer Timeline and Audit views over existing immutable evidence, including source references and deep links | Normalize remaining old/new values, device identity (CORE-20 makes the login evidence addressable per user, but entries carry a source address, not a device), generalized approvals and document-access evidence. Trail pagination is now contract-declared and reachable from the generated client. | All final domain events |
