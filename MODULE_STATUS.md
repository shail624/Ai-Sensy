...
| Settings | 72% | Organization/application/flags/preferences routes complete, plus a permission-aware Tags administration panel | Approved business/WhatsApp hours/messages, assignment/auto-resolve/read receipts, campaign/opt-in, pipeline/SLA/notifications/security/audit controls; canned-message and attribute-definition management remain API-only. | Domain configuration APIs, RBAC |
...
| Tags and Attributes | 92% | Tag CRUD is now reachable in the product through the Settings → Tags panel (search, usage filter, create/rename/recolour/delete, usage counts, permission-aware controls); contact links, custom attributes and filters complete | Attribute **definitions** remain API-only with no management surface, so the attribute half of this row is not complete. Required/active controls and final domain-specific fields; preserve existing model. | Settings, domain schemas |
