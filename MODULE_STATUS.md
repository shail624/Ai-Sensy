...
CORE-03 connected the existing Reactivation route to the real CORE-02 domain authority with factual
pipeline projections, governed transitions, assignments, evidence, shared Tasks/Documents/Customer
360, and responsive accessible board/list workflows. It did not build the later KYC, SIM, or
Activation operational screens.
...
| Reactivation | 70% | CORE-03 real persisted dashboard, fifteen-stage Kanban/list, governed drag/keyboard moves, assignments, eligibility/rejection, history/notes, Tasks/Documents/Customer 360 reuse, reservation/family/conversion/SLA evidence, and responsive states complete | Saved views/pagination at scale, notification automation, KYC/SIM/Activation operational convergence, analytics and final visual regression with representative production data. | KYC, SIM Orders, Activation, Notifications, Analytics |
...
| API | 83% | 184-path OpenAPI 3.1 contract; CORE-03 adds the joined pipeline projection and immutable case-note paths over CORE-02 | Remaining final-domain routes, usage logs/IP restrictions completeness, key regeneration/revocation UX, published documentation. | Each domain milestone, Download Center |
...
| Audit Timeline | 84% | CORE-03 retains CORE-02 transition evidence and adds immutable internal-note audit/Customer Timeline projection through existing authorities | Normalize remaining old/new values, device/login, generalized approvals, document access and assignment evidence. | All final domain events |
