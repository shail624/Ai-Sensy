"""Typed KYC adapter over the shared workspace-view authority."""
...
from app.schemas.vi_domain import KycViewCreate
from app.services.audit_service import AuditAction
from app.services.workspace_view_service import WorkspaceViewPolicy, WorkspaceViewService

MANAGE_SHARED_PERMISSION = "kyc:views_manage"
POLICY = WorkspaceViewPolicy(
    workspace="kyc",
    label="KYC",
    manage_permission=MANAGE_SHARED_PERMISSION,
    created_action=AuditAction.KYC_VIEW_CREATED,
    deleted_action=AuditAction.KYC_VIEW_DELETED,
    entity_type="kyc_view",
)


class KycViewService:
    ...
    async def create(self, actor: User, payload: KycViewCreate) -> WorkspaceView:
