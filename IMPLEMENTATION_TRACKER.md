...
_Last updated: 2026-08-08 · QR-08 Unified Inbox integration, on top of QR-01 WAHA provider adapter foundation, the QR-00 provider selection and provider-message identity foundation, the MySQL migration evidence hardening, the Alembic version-table MySQL fix (long revision ids), the Chat History pagination/polling/accessibility hardening, the Dedicated Chat History workspace, the User Attributes remediation, its test-hardening follow-up and M13-06B. Provider certification still blocks every live history, media, event and adapter behavior._
...
- **Release:** `1.0.0-rc1`
- **Migration/OpenAPI:** `0043_conversation_channel_endpoints` (44 revisions; +1, additive expand-only) · **207 paths** (206 through QR-07; QR-08 adds `POST /webhooks/waha`, the WAHA analogue of the existing `/webhooks/whatsapp` route — QR-04 built its verify/parse/dedupe logic in 2026 but never wired an HTTP route to it) — RBAC catalog unchanged (reuses `inbox:read`/`inbox:write`/`inbox:assign`/`messages:send`, `channels:read`/`channels:authenticate`)
...
- **Next milestone:** `QR-09 — production validation`. Not started.
