...
import { useCreateKycView, useDeleteKycView, useKycViews } from "@/features/kyc/api";
import type { KycStatus, KycView, KycViewCreate } from "@/features/kyc/types";
import { apiErrorMessage } from "@/lib/api/errors";
import { useHasPermission } from "@/lib/auth";

export interface KycPortableFilters {
  q: string;
  status: KycStatus | "";
}

interface Props {
  filters: KycPortableFilters;
  onApply: (filters: KycPortableFilters) => void;
}

function portableFilters(filters: KycPortableFilters): KycViewCreate["filters"] {
  return {
...
export function KycSavedViews({ filters, onApply }: Props): JSX.Element {
  const [open, setOpen] = useState(false);
  const views = useKycViews();
  const canManageShared = useHasPermission("kyc:views_manage");

  function apply(saved: KycView): void {
    onApply({ q: saved.filters.q ?? "", status: saved.filters.status ?? "" });
  }
    ...
    <>
      <div className="flex min-h-11 w-full flex-wrap items-center gap-2 rounded-xl border border-border bg-surface px-3 py-2 shadow-sm">
        ...
        {views.isLoading ? (
          ...
          <div
            aria-label="Personal and team KYC views"
          ...
          <span className="min-w-0 flex-1 text-xs text-text-disabled">
            Save this KYC queue filter for one-click reuse.
          </span>
        )}
        <Button
          variant="secondary"
...
interface DialogProps {
  filters: KycViewCreate["filters"];
  views: KycView[];
  canManageShared: boolean;
  onClose: () => void;
}

function KycViewsDialog({ filters, views, canManageShared, onClose }: DialogProps): JSX.Element {
  const [name, setName] = useState("");
  const [visibility, setVisibility] = useState<"private" | "shared">("private");
  const [pendingDelete, setPendingDelete] = useState<KycView | null>(null);
  const createView = useCreateKycView();
  const deleteView = useDeleteKycView();
  ...
  return (
    <Modal title="KYC saved views" variant="sheet" onClose={onClose}>
      <form className="space-y-4" onSubmit={save}>
        <div className="grid gap-3 sm:grid-cols-[1fr_160px]">
          <Field htmlFor="kyc-view-name" label="View name">
            <Input
              id="kyc-view-name"
              autoFocus
              value={name}
              ...
              placeholder="Example: Documents pending"
            />
          </Field>
          <Field htmlFor="kyc-view-visibility" label="Visible to">
            <Select
              id="kyc-view-visibility"
              value={visibility}
              onChange={(event) =>
        ...
        {!canManageShared ? (
          <p className="text-xs text-text-disabled">
            Your role can save personal views. A KYC manager can publish team views.
          </p>
        ) : null}
        {createView.error ? (
      ...
      {pendingDelete ? (
        <div className="mt-4 rounded-lg border border-danger/30 bg-danger/5 p-3">
          ...
          <p className="mt-1 text-xs text-text-secondary">
            This removes the saved definition only. No KYC case is changed.
          </p>
          {deleteView.error ? (
