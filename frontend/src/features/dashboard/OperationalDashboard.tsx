  ...
  return (
    <section aria-labelledby="attention-summary-title" className="overflow-hidden rounded-surface border border-border bg-surface shadow-sm">
      <div className="border-b border-border px-4 py-3">
        <h2 id="attention-summary-title" className="text-sm font-semibold text-text-primary">Needs your attention</h2>
        <p className="mt-0.5 text-xs text-text-secondary">Click any number to see the list.</p>
      </div>
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 [&>*:nth-last-child(-n+5)]:xl:border-b-0">
        {permissions.reactivation ? <SignalCell icon={ShieldAlert} label="Blocked customers" value={blocked} detail="Late, missing documents or not reachable" href="/reactivation/pipeline" loading={queryState.reactivation.loading} error={queryState.reactivation.error} tone={blocked > 0 ? "danger" : "neutral"} /> : null}
        {permissions.kyc ? <SignalCell icon={FileWarning} label="KYC reviews pending" value={pendingKyc} detail="Waiting for a reviewer" href="/reactivation/kyc" loading={queryState.kyc.loading} error={queryState.kyc.error} tone={pendingKyc > 0 ? "warning" : "neutral"} /> : null}
        {permissions.reactivation ? <SignalCell icon={PackageCheck} label="SIM delivery risk" value={delayedSim} detail="SIM needed and running late" href="/reactivation/pipeline" loading={queryState.reactivation.loading} error={queryState.reactivation.error} tone={delayedSim > 0 ? "danger" : "neutral"} /> : null}
        {permissions.reactivation ? <SignalCell icon={Smartphone} label="Activations overdue" value={overdueActivations} detail="Waiting to be activated, running late" href="/reactivation/pipeline" loading={queryState.reactivation.loading} error={queryState.reactivation.error} tone={overdueActivations > 0 ? "danger" : "neutral"} /> : null}
        {permissions.campaigns ? <SignalCell icon={Send} label="Campaigns need action" value={campaignActions} detail="Failed, paused or some messages failed" href="/campaigns" loading={queryState.campaigns.loading} error={queryState.campaigns.error} tone={campaignActions > 0 ? "warning" : "neutral"} /> : null}
        {permissions.inbox ? <SignalCell icon={MessageSquareReply} label="Replies waiting" value={replies} detail="Chats with unread messages" href="/inbox" loading={queryState.inbox.loading} error={queryState.inbox.error} tone={replies > 0 ? "warning" : "neutral"} /> : null}
        {permissions.templates ? <SignalCell icon={BadgeAlert} label="Templates blocked" value={templateActions} detail="Rejected, paused or turned off by WhatsApp" href="/templates" loading={queryState.templates.loading} error={queryState.templates.error} tone={templateActions > 0 ? "warning" : "neutral"} /> : null}
        {permissions.reactivation || permissions.kyc ? <SignalCell icon={UserRoundCog} label="Agents need attention" value={agents} detail="Team members with late or stuck work" href="/reactivation/pipeline" loading={queryState.reactivation.loading || queryState.kyc.loading} error={queryState.reactivation.error && queryState.kyc.error} tone={agents > 0 ? "info" : "neutral"} /> : null}
    ...
    <Card padding={false} className="overflow-hidden">
      <div className="border-b border-border px-4 py-4 sm:px-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h2 className="text-base font-semibold text-text-primary">What to do next</h2>
            <p className="mt-1 text-xs text-text-secondary">Most urgent first.</p>
          </div>
          <Badge tone={items.some((item) => item.severity === "critical") ? "danger" : items.length > 0 ? "warning" : "success"} dot>
            {items.length > 0 ? `${items.length} to do` : "Nothing to do"}
          </Badge>
        </div>
      ...
      {loading && items.length === 0 ? (
        ...
        <div className="p-7">
          <EmptyState compact icon={<CheckCircle2 className="h-6 w-6" />} title={unavailable.length > 0 ? "Some lists could not load" : "All caught up"} description={unavailable.length > 0 ? "Press Refresh to try again." : "Nothing is late, stuck, failed or unread."} />
        </div>
      ) : (
        <>
...
function MyWorkSnapshot({ userId }: { userId: string | undefined }): JSX.Element {
  ...
  return (
    <Card padding={false} className="overflow-hidden">
      ...
      {stats.isError ? <div className="p-4"><ErrorState message={apiErrorMessage(stats.error)} onRetry={() => void stats.refetch()} /></div> : tasks.isLoading ? <div className="space-y-2 p-4">{Array.from({ length: 3 }, (_, index) => <Skeleton key={index} className="h-20 w-full" />)}</div> : tasks.isError ? <div className="p-4"><ErrorState message={apiErrorMessage(tasks.error)} onRetry={() => void tasks.refetch()} /></div> : rows.length === 0 ? <div className="p-5"><EmptyState compact title="No overdue task" description="See today’s and upcoming tasks in Tasks." /></div> : <ul className="divide-y divide-border">{rows.map((task) => <li key={task.id} className="p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-semibold text-text-primary">{task.title}</p><p className="mt-0.5 truncate text-xs text-text-secondary">{task.contact_name ?? "No customer linked"}</p><div className="mt-2 flex flex-wrap items-center gap-1"><TaskPriorityChip value={task.priority} /><TaskDue task={task} /></div></div><TaskActions task={task} /></div></li>)}</ul>}
    </Card>
  );
}
...
function BlockedCustomers({ cards, loading, error, onRetry }: { cards: ReturnType<typeof blockedReactivationCards>; loading: boolean; error: unknown; onRetry: () => void }): JSX.Element {
  return (
    <Card padding={false} className="overflow-hidden">
      <CardHeader className="border-b border-border px-4 py-4 sm:px-5" title="Blocked customers" description="Customers who are stuck, and why" icon={<UsersRound aria-hidden className="h-4 w-4" />} action={<Link to="/reactivation/pipeline" className="inline-flex min-h-9 items-center gap-1 rounded-control px-2 text-xs font-semibold text-accent hover:bg-accent-soft">Open pipeline<ArrowRight className="h-3.5 w-3.5" /></Link>} />
      {loading ? <div className="space-y-2 p-4">{Array.from({ length: 4 }, (_, index) => <Skeleton key={index} className="h-20 w-full" />)}</div> : error ? <div className="p-4"><ErrorState message={apiErrorMessage(error)} onRetry={onRetry} /></div> : cards.length === 0 ? <div className="p-5"><EmptyState compact title="No stuck customers" description="Everyone is on track." /></div> : <ul className="divide-y divide-border">{cards.slice(0, 6).map((card) => <li key={card.id} className="p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-semibold text-text-primary">{card.contact_name}</p><p className="mt-0.5 text-xs text-text-secondary">{REACTIVATION_STAGE_LABELS[card.stage]} · {card.owner_name ?? "Unassigned"}</p></div>{card.sla_status === "breached" ? <Badge tone="danger">SLA breached</Badge> : <Badge tone="warning">Blocked</Badge>}</div><div className="mt-2 flex flex-wrap gap-1">{reactivationReasons(card).map((reason) => <Badge key={reason} tone={reason === "SLA breached" ? "danger" : "warning"}>{reason}</Badge>)}</div></li>)}</ul>}
    </Card>
  );
}
  ...
  return (
    <Card padding={false} className="overflow-hidden">
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-border px-4 py-4 sm:px-5">
        <div><div className="flex items-center gap-2"><BarChart3 aria-hidden className="h-4 w-4 text-accent" /><h2 className="text-base font-semibold text-text-primary">Today vs yesterday</h2></div><p className="mt-1 text-xs text-text-secondary">How today compares with the day before.</p></div>
...
export function OperationalDashboard(): JSX.Element {
  ...
  if (visibleSources === 0) {
    return <EmptyState icon={<Activity className="h-7 w-7" />} title="Nothing to show here yet" description="Your role does not include access to these lists. Ask your admin." />;
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-control border border-border bg-surface-2 px-3 py-2.5">
        <p className="text-xs leading-5 text-text-secondary">Press Refresh to see the latest numbers.</p>
        <Button variant="secondary" size="sm" leftIcon={<RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} />} onClick={refreshAll}>Refresh</Button>
      </div>
