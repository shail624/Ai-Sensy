...
import { useCampaigns, usePhoneNumbers } from "@/features/campaigns/api";
import { fromSharedFilters, useHistoryViews } from "@/features/chat-history/api";
import { HistoryFiltersDialog } from "@/features/chat-history/HistoryFiltersDialog";
import { TranscriptExportDialog } from "@/features/chat-history/TranscriptExportDialog";
import {
  apiErrorMessage,
  useAssignableUsers,
  useConversation,
...
function readFilters(params: URLSearchParams): InboxFilters {
  return {
    ...
    hasMedia: params.get("media") === "true" || undefined,
    hasAudit: params.get("audit") === "true" || undefined,
  };
}
...
function writeParams(
  ...
  if (filters.hasAudit) params.set("audit", "true");
 ...
 * job; this route only reproduces the factual record for lookup, review and audit reference.
 * Transcript generation is a separate, permission-scoped worker flow over the persisted ledger,
 * so exporting never depends on how many message pages the browser has loaded.
 */
export function ChatHistory(): JSX.Element {
  ...
  const canExport = useHasPermission("inbox:export");
  const canReadCampaigns = useHasPermission("campaigns:read");
  const canManageViews = useHasPermission("inbox:views_manage");
  const campaigns = useCampaigns(canReadCampaigns);
  const historyViews = useHistoryViews();
    ...
    <div className="flex h-full min-h-0 flex-col bg-canvas">
        ...
        >
          <div className="space-y-3 border-b border-border p-3">
            ...
            <div className="flex items-center gap-2">
              <Button
                type="button"
                variant={advancedFilterCount > 0 ? "subtle" : "secondary"}
                size="sm"
              ...
              {(historyViews.data ?? []).length > 0 ? (
                <div
                  aria-label="Team shared Chat History views"
                  className="flex min-w-0 flex-1 gap-1 overflow-x-auto"
                >
                  {(historyViews.data ?? []).map((view) => (
                    <button
                      key={view.id}
                      type="button"
                      onClick={() => applyFilters(fromSharedFilters(view.filters))}
                      className="inline-flex h-8 shrink-0 items-center gap-1.5 rounded-full border border-border bg-surface-2 px-2.5 text-[11px] font-semibold text-text-primary hover:border-accent"
                    >
                ...
                <span className="truncate text-[11px] text-text-disabled">
                  Date, campaign, media and audit filters
                </span>
              )}
            </div>
      ...
      {filtersOpen ? (
        <HistoryFiltersDialog
          ...
          savedViews={historyViews.data ?? []}
          canReadCampaigns={canReadCampaigns}
          canAudit={canAudit}
      ...
      {exportOpen && selectedId && thread ? (
        <TranscriptExportDialog
          conversationId={selectedId}
          contactName={contactLabel(thread.contact)}
