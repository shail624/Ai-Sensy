    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Section title="Scheduled follow-ups" description="Open Task records remain the reminder authority." icon={<CalendarClock aria-hidden className="h-4 w-4" />}>
          {record.reminders.length === 0 ? <EmptyState compact title="No open reminder" description="Follow-up and release-date reminders for this customer appear here." /> : <ul className="space-y-2">{record.reminders.slice(0, 5).map((reminder) => <li key={reminder.id} className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-border bg-surface-2 px-3 py-2.5"><div className="min-w-0"><p className="truncate text-sm font-medium text-text-primary">{reminder.title}</p><p className="mt-0.5 text-xs text-text-secondary">Due {new Date(reminder.due_at).toLocaleString()}</p></div><Badge tone={new Date(reminder.due_at) < new Date() ? "danger" : "warning"}><AlarmClock aria-hidden className="h-3 w-3" />{new Date(reminder.due_at) < new Date() ? "Overdue" : "Scheduled"}</Badge></li>)}</ul>}
        </Section>

        <Section title="Internal case notes" description="Notes your team wrote on this customer's case. They cannot be edited later." icon={<History aria-hidden className="h-4 w-4" />}>
      ...
      <section aria-labelledby="domain-facts-title" className="overflow-hidden rounded-2xl border border-border bg-surface shadow-sm">
        <header className="border-b border-border px-4 py-3.5"><h3 id="domain-facts-title" className="text-sm font-semibold text-text-primary">Protected domain facts</h3><p className="mt-0.5 text-xs text-text-secondary">For reading only. To change something, open it in its own page.</p></header>
