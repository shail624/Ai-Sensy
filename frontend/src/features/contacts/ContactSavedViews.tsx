...
import type { ContactView, ContactViewCreate } from "@/features/contacts/types";
...
function portableFilters(filters: ContactFilters): ContactViewCreate["filters"] {
  return {
    q: filters.search.trim() || undefined,
    tag_id: filters.tagId || undefined,
    attributes: Object.fromEntries(
      Object.entries(filters.attributes).filter(([, value]) => Boolean(value)),
    ),
  };
}

export function ContactSavedViews({ filters, onApply }: Props): JSX.Element {
  const [open, setOpen] = useState(false);
  const views = useContactViews();
  const canManageShared = useHasPermission("contacts:views_manage");

  function apply(saved: ContactView): void {
    onApply({
      search: saved.filters.q ?? "",
      tagId: saved.filters.tag_id ?? "",
    ...
    <>
      <div className="flex min-h-11 w-full flex-wrap items-center gap-2 rounded-xl border border-border bg-surface px-3 py-2 shadow-sm">
        ...
        {views.isLoading ? (
          ...
          <div
            aria-label="Personal and team Contacts views"
          ...
          <span className="min-w-0 flex-1 text-xs text-text-disabled">
            Save this contact search for one-click reuse.
          </span>
        )}
        <Button
          variant="secondary"
...
interface DialogProps {
  filters: ContactViewCreate["filters"];
  views: ContactView[];
  canManageShared: boolean;
  onClose: () => void;
}

function ContactViewsDialog({
  filters,
  views,
  canManageShared,
  onClose,
  ...
  const createView = useCreateContactView();
  const deleteView = useDeleteContactView();
  ...
  return (
    <Modal title="Contacts saved views" variant="sheet" onClose={onClose}>
      <form className="space-y-4" onSubmit={save}>
        <div className="grid gap-3 sm:grid-cols-[1fr_160px]">
          <Field htmlFor="contact-view-name" label="View name">
            <Input
              id="contact-view-name"
              autoFocus
              value={name}
              maxLength={80}
              onChange={(event) => setName(event.target.value)}
              placeholder="Example: VIP follow-ups"
            />
          </Field>
          <Field htmlFor="contact-view-visibility" label="Visible to">
            <Select
              id="contact-view-visibility"
              value={visibility}
              onChange={(event) =>
        ...
        {!canManageShared ? (
          <p className="text-xs text-text-disabled">
            Your role can save personal views. A Contacts manager can publish team views.
          </p>
        ) : null}
        {createView.error ? (
      ...
      {pendingDelete ? (
        <div className="mt-4 rounded-lg border border-danger/30 bg-danger/5 p-3">
          ...
          <p className="mt-1 text-xs text-text-secondary">
            This removes the saved definition only. No contacts are changed.
          </p>
          {deleteView.error ? (
...
interface SavedViewGroupProps {
  ...
  empty: string;
  views: ContactView[];
  onDelete: (view: ContactView) => void;
}
