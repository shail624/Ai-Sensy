...
function DocumentsPanel({ card }: { card: KycOperationsCard }): JSX.Element {
  ...
  return <div role="tabpanel" className="space-y-4"><section className="rounded-xl border border-border p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="text-sm font-semibold text-text-primary">Document checklist</h3><p className="mt-1 text-xs text-text-secondary">References point to protected Document Center records. Aadhaar and PAN numbers are never collected here.</p></div><Badge tone={card.checklist_complete ? "success" : "warning"}>{card.checklist_complete ? "Complete" : "Missing evidence"}</Badge></div>{!canReadDocuments ? <div className="mt-3"><EmptyState compact title="Protected document access denied" description="Your role can view KYC status but cannot inspect or link customer documents." /></div> : documents.isLoading ? <div className="mt-3 space-y-2"><Skeleton className="h-20 w-full" /><Skeleton className="h-20 w-full" /></div> : documents.isError ? <div className="mt-3"><ErrorState message={apiErrorMessage(documents.error)} onRetry={() => void documents.refetch()} /></div> : <div className="mt-3 space-y-3">{(["aadhaar", "pan"] as KycDocumentPurpose[]).map((purpose) => { const current = card.checklist.find((item) => item.purpose === purpose); return <div key={purpose} className="rounded-xl border border-border bg-surface-2 p-3"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-sm font-semibold text-text-primary">{KYC_DOCUMENT_LABELS[purpose]}</p><p className="mt-0.5 text-xs text-text-secondary">{current ? `${current.document_title} · ${current.document_status}` : "No protected reference linked"}</p></div><Badge tone={current?.document_status === "verified" ? "success" : current ? "warning" : "neutral"}>{current?.document_status ?? "Missing"}</Badge></div>{canWrite ? <div className="mt-3 flex gap-2"><label className="min-w-0 flex-1 text-xs font-medium text-text-secondary"><span className="sr-only">Select {KYC_DOCUMENT_LABELS[purpose]}</span><select className="h-10 w-full rounded-lg border border-border bg-surface px-3 text-sm text-text-primary" defaultValue={current?.document_id ?? ""} onChange={(event) => { if (event.target.value) setReference.mutate({ kycId: card.id, purpose, documentId: event.target.value, expectedRowVersion: card.row_version }); }}><option value="">Select document</option>{options.map((document) => <option key={document.id} value={document.id}>{document.title} · {document.status}</option>)}</select></label></div> : null}</div>; })}{options.length === 0 ? <EmptyState compact title="No identity documents available" description="Add Aadhaar/PAN evidence through the protected Document Center, then link it here." /> : null}</div>}{setReference.error ? <div className="mt-3"><ErrorState message={apiErrorMessage(setReference.error)} /></div> : null}</section><section className="rounded-xl border border-border p-3"><DocumentWorkspace contactId={card.contact_id} compact /></section></div>;
}
